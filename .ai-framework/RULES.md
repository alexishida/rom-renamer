...
- O fluxo principal deve funcionar offline com catalogo SQLite local, sem depender de consultas externas.
...
- Preferir alteracoes pequenas, claras, reversiveis e consistentes com padroes ja usados.
...
- Operacoes sensiveis devem passar por IPC com validacao de entrada no processo main.
- Regras de negocio de ROM devem ficar fora de componentes React sempre que possivel.
- `src/main/rom/dat.ts` concentra catalogo SQLite, importacao DAT/XML, busca manual e matching por hash e fuzzy.
- `src/main/rom/region.ts` concentra deteccao de regiao por header binario; suporta Nintendo 64, Game Boy, Game Boy Color, Game Boy Advance e Nintendo DS.
- `scripts/build-rom-catalog.mjs` gera catalogo bundled em `resources/rom-catalog.sqlite`; parser e schema devem permanecer compativeis com runtime.
- Base bundled nasce de DATs locais em `dat/`; arquivos temporarios em `temp/` nao devem virar dependencia permanente do fluxo.
...
## Identificacao e catalogo

- Pipeline de identificacao deve seguir esta ordem: detectar plataforma, calcular hashes, consultar catalogo SQLite local por hash, tentar fuzzy por nome e so entao usar fallback por header.
- Override manual de plataforma deve vencer deteccao automatica por extensao.
- Busca automatica por hash no catalogo deve seguir esta ordem fixa: `SHA-1`, depois `MD5`, depois `CRC32`.
- Match exato por hash e fonte de maior confianca (`high`).
...
- DATs, XMLs e APIs externas nao devem ser consultados diretamente no scan; scan consulta apenas SQLite local.
...
- Busca manual pode usar contexto de plataforma para reduzir resultados irrelevantes, mas sem impedir fallback ao catalogo completo quando nao houver arquivo compativel carregado.
- Limpar catalogo ou remover arquivo importado deve ser acao explicita da UI e reportar contagem removida.

## Fluxo de arquivo e rename
...
- Estados `pending`, `identifying`, `identified`, `validated`, `ignored`, `renamed` e `error` devem permanecer coerentes entre UI, store e main.
